...
import org.bardframework.commons.reflection.JavaClassFinder;
import org.bardframework.commons.util.FileUtils;
...
public class ReloadableConfig {
    ...
    private List<String> reloadableConfigPath;
    ...
    private void reload() throws IOException {
        ...
        for (String path : reloadableConfigPath) {
            resources.addAll(Arrays.asList(FileUtils.getResources(path)));
        }
    ...
    private List<String> check(Properties newConfigs) {
        ...
        for (Class<? extends ConfigKey> aClass : new JavaClassFinder().findAllMatchingTypes(ConfigKey.class)) {
            if (!aClass.isEnum()) {
                continue;
            }
