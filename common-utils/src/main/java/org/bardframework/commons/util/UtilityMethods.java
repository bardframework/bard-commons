...
public final class UtilityMethods {
    ...
    public static String validateIP(String ip) {
        ...
        try {
            for (String num : ip.split("\\.")) {
                ...
                builder.append(Constants.DOT_CHAR);
            }
    ...
    public static String numberToAlphabet(long number) {
        ...
        String tempStringNumber = tempStringNumberOrgin.replaceAll("-", Constants.EMPTY_STRING);
        ...
        if ("-".equals(tempStringNumberOrgin.substring(0, 1))) {
            ...
            outString.append(Constants.SPACE_CHAR);
    ...
    private static void addPartFullString(List<String> partFullString, String partInWord, int partCount, int index) {
        ...
        if (index == partCount - 1) {
            partPreFix = Constants.EMPTY_STRING;
        }
        if (index == 0) {
            if (Constants.EMPTY_STRING.equals(partInWord)) {
                partFullString.add(index, Constants.EMPTY_STRING);
            } else {
    ...
    private static void addParts(List<String> partFullString, String partInWord, String partPreFix, int index) {
        if (!Constants.EMPTY_STRING.equals(partFullString.get(index - 1))) {
            if (!Constants.EMPTY_STRING.equals(partInWord)) {
                partFullString.add(index, SPLITTER + partInWord + Constants.SPACE_STRING + partPreFix);
            } else {
                partFullString.add(index, Constants.EMPTY_STRING);
            }
        } else {
            if (!Constants.EMPTY_STRING.equals(partInWord)) {
                partFullString.add(index, SPLITTER + partInWord + Constants.EMPTY_STRING + partPreFix);
            } else {
                partFullString.add(index, Constants.SPACE_STRING);
            }
        }
    }
