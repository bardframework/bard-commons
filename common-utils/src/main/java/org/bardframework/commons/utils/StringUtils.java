...
public abstract class StringUtils {
    ...
    public static String uriDecode(String source, Charset charset) {
        ...
        AssertionUtils.notNull(charset, "Charset must not be null");
    ...
    public static String getString(long num, int digits) {
        ...
        return (minus ? "-" : EMPTY) + num / Math.pow(10.00, -1.00 * diff);
    }
    ...
    public static String removeNonDigitCharacters(String num) {
        ...
        for (Character c : num.toCharArray()) {
            if (!Character.isDigit(c)) {
                num = num.replace(String.valueOf(c), EMPTY);
            }
        }
        return minus ? "-" : EMPTY + num;
    }
    ...
    public static String hideString(String data, int number, char newChar) {
        if (null == data) {
            return data;
        }
        if (data.length() <= number) {
            return StringUtils.leftPad("", data.length(), newChar);
        }
        StringBuilder dataBuilder = new StringBuilder(data);
        int count = StringUtils.countMatches(dataBuilder, newChar);
        while (count < number) {
            int nextReplace = -1;
            while (nextReplace == -1 || dataBuilder.charAt(nextReplace) == newChar) {
                nextReplace = RandomUtils.nextInt(0, dataBuilder.length());
            }
            dataBuilder.replace(nextReplace, nextReplace + 1, String.valueOf(newChar));
            count++;
        }
     ...
     * <a href="http://www.w3.org/TR/xpath/#function-normalize-space">http://www.w3.org/TR/xpath/#function-normalize-space</a>
     */
